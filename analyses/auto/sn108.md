# Subnet 108 — TalkHead

> Auto-analyzed: 2026-05-08 06:05 PDT · easy-entry score: 22.5/100 · refreshes hourly

*This analysis is auto-generated from live chain data. It updates every hour.
For hand-curated notes, check the Bittensor Discord or the subnet's own docs.*

## Quick stats

| Metric | Value |
|---|---|
| Category | other |
| GPU need | ? |
| Reward shape | winner |
| Active miners | 1 |
| UID slots | 131/256 (125 free) |
| Burn fee | 0.60155 τ `[░░░░░░░░░░░░░░░░░░░░]` 0.1% of max |
| Burn min / max | 0.50000 τ → 100.0000 τ |
| Emission / day | 52.6461 τ |
| Top-1 share | 100.0% |
| Liquidity (TAO in) | 2,069.2 τ |
| Price (τ/α) | 0.004875 |
| Age | 144 days |


> **Trend (from 57 snapshots):** Burn fee has risen ~20% over the last 57 snapshots.

## Easy-entry score: 22 / 100  🔴 low

- top1 takes 100% (winner-take-all)
- 125 free UID slots
- cheap burn fee 0.6016 τ

| Component | Score |
|---|---|
| GPU friction | 0.3 / 20 |
| Decentralization (top-1) | 0.0 / 20 |
| Active miners | 0.0 / 15 |
| Free slots | 0.5 / 15 |
| Burn fee | 0.8 / 15 |
| Liquidity | 0.0 / 10 |
| Emission | 0.0 / 5 |

## What is known about this category

Category information is limited. Check the subnet's GitHub and Discord for specifics before investing time or registration fees.

## GPU / hardware

GPU need listed as **?**.

## Reward shape

Reward shape is not yet categorised. Check the subnet's incentive mechanism docs to understand how emissions are distributed.

## Cost to operate a miner

✅ no paid API keys required · 💻 no GPU required · ℹ️ baseline estimate (check repo to verify)

**Monthly recurring:** ~$50 – $1,500 · **One-time setup:** ~$185 – $222

### Monthly costs

| Item | Required | USD / month | Notes |
|---|:-:|---|---|
| Hosting / VPS | ✓ | $50 – $1,500 | depends on chosen model size |

### One-time costs

| Item | Required | USD | Notes |
|---|:-:|---|---|
| Registration burn fee (one-time per UID) | ✓ | $185 – $222 | 0.6016 τ · @ $307.36/τ · includes ~20% buffer for fee jitter |

> Estimates are baseline ranges from the subnet's category and GPU need; check the subnet's `env.example` / README for specific API key requirements.

## Getting started (generic)

1. Search for this subnet's GitHub/Discord using the links below.
2. Set up a Bittensor wallet: hot+cold key pair.
3. Fund with burn fee + buffer (~0.7219 τ recommended).
4. Clone the subnet repo, run the miner in test mode first.
5. `btcli subnet register --netuid 108 --wallet.name <cold> --wallet.hotkey <hot>`
6. Monitor your score during the immunity window.

## Links

- GitHub: <https://github.com/talkheadai/talkhead-subnet>
- TAO.app: <https://tao.app/subnet/108>

